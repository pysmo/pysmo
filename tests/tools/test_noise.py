...
@pytest.mark.mpl_image_compare(remove_text=True, baseline_dir='../baseline/', style='default')
def test_peterson():  # type: ignore
    ...
    assert all(nm_03.T == np.array([0.10, 0.17, 0.22, 0.32, 0.40, 0.80, 1.24, 2.40, 3.80,
                                    4.30, 4.60, 5.00, 6.00, 6.30, 7.90, 10.00, 12.00,
                                    15.40, 15.60, 20.00, 21.90, 31.60, 45.00, 70.00,
                                    101.00, 154.00, 328.00, 354.80, 600.00, 10**4,
                                    10**5]))
    fig = plt.figure()
...
def test_generate_noise():  # type: ignore
    ...
    nhnm_velo = noise.NoiseModel(psd=nhnm.psd+20*np.log10(nhnm.T/2/np.pi), T=nhnm.T)

    nhnm_data_acc = noise.generate_noise(model=nhnm, npts=npts, sampling_rate=srate,
                                         seed=0).data
    nhnm_data_vel = noise.generate_noise(model=nhnm, npts=npts, sampling_rate=srate,
                                         return_velocity=True, seed=0).data
    freqs_acc, power_acc = signal.welch(nhnm_data_acc, sfrec, nperseg=nperseg, nfft=nfft,
                                        scaling='density')
    freqs_vel, power_vel = signal.welch(nhnm_data_vel, sfrec, nperseg=nperseg, nfft=nfft,
                                        scaling='density')
