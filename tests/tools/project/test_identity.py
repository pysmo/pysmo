...
class TestEntryIdentity:
    ...
    def test_identity_is_memoised_and_reset_on_field_change(self) -> None:
        ...
        assert entry.identity != first  # components changed, cache recomputed
    ...
    def test_identity_recomputes_when_a_nested_field_is_mutated(self) -> None:
        station = MiniStation(
            name="ANMO",
            network="IU",
            location="00",
        ...
        entry.station.channel = "HHZ"  # mutate the nested Station, not the field
        assert entry.identity != before
