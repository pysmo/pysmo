...
def _checksum(seismogram: Seismogram) -> str:
    """Checksum a freshly downloaded seismogram, before `transform` runs.

    Hashed pre-`transform` deliberately: `transform`'s output (e.g. a
    ...
    `transform` would pick up that unrelated mutation as false "drift".
    """
    h = hashlib.sha256()
