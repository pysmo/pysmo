...
def _validate_sacenum(instance: "SacIOBase", attribute: Attribute, value: Any) -> None:
...
def _validate_with_iztype(
    instance: "SacIOBase", attribute: Attribute, value: int | float | None
) -> None:
...
def _null_if_undefined(value: Any, header_name: str, optional: bool) -> Any:
    ...
    if not optional:
        raise ValueError(
            ...
            f"header and can never legally be unset."
        )
    warnings.warn(
        f"{header_name!r} was set to {value!r}, SAC's 'undefined' value for "
        f"this header type. Treating it as unset (None): a file always "
        f"writes None as {value!r} and reads {value!r} back as None, so "
        f"storing the literal value here would silently diverge from that.",
        UserWarning,
        stacklevel=2,
    )
    return None
