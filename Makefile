.PHONY: help check-uv build clean docs format format-check lint python sync test-figs tests upgrade
...
test-figs: check-uv ## Regenerate matplotlib baseline images.
	uv run pytest tests/test_sacfunc.py --mpl-generate-path=tests/baseline

tests: check-uv ## Run all tests with pytest.
	uv run pytest --cov=pysmo --cov-report=term-missing --mpl
