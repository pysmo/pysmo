...
def calc_power(data, sampling_frequency):
    ...
    freqs, psd = signal.welch(data, sampling_frequency, nperseg=nperseg, nfft=nfft,
                              scaling='density')
...
low_noise_seismogram = generate_noise(npts=npts, model=low_noise_model,
                                      sampling_rate=delta)
mid_noise_seismogram = generate_noise(npts=npts, model=mid_noise_model,
                                      sampling_rate=delta)
high_noise_seismogram = generate_noise(npts=npts, model=high_noise_model,
                                       sampling_rate=delta)
...
# Plot PSD of noise
plt.subplot(122)
plt.plot(1/f_high, 10*np.log10(Pxx_dens_high), 'r', linewidth=0.5, label='generated high noise')
plt.plot(1/f_mid, 10*np.log10(Pxx_dens_mid), 'g', linewidth=0.5, label='generated medium noise')
plt.plot(1/f_low, 10*np.log10(Pxx_dens_low), 'b', linewidth=0.5, label='generated low noise')
plt.plot(high_noise_model.T, high_noise_model.psd, 'k', linewidth=1, linestyle='dotted',
         label='NHNM')
plt.plot(mid_noise_model.T, mid_noise_model.psd, 'k', linewidth=1, linestyle='dashdot',
         label='Interpolated noise model')
plt.plot(low_noise_model.T, low_noise_model.psd, 'k', linewidth=1, linestyle='dashed',
         label='NLNM')
